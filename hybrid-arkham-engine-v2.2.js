/* ============================================================================
   ...
   - MEDIUM: Added optional heat floor to prevent excessive debt
   - MEDIUM: Hidden HP/SP status during grace period for clarity
   - Full value validation with rational min/max limits
   - AI Dungeon scripting guide compliance
...
state.initialHeatValue = 0                          // Range: 0-100
state.initialTemperatureValue = 1                   // Range: 1-50
state.temperatureIncreaseChance = 15                // Range: 1-100 (% chance)
state.heatIncreaseValue = 1                         // Range: 0-10
state.temperatureIncreaseValue = 1                  // Range: 0-5

state.playerIncreaseHeatImpact = 2                  // Range: 0-20
state.playerDecreaseHeatImpact = 2                  // Range: 0-20
state.playerIncreaseTemperatureImpact = 1           // Range: 0-10
state.playerDecreaseTemperatureImpact = 1           // Range: 0-10
state.threshholdPlayerIncreaseTemperature = 2       // Range: 1-20
state.threshholdPlayerDecreaseTemperature = 2       // Range: 1-20

state.modelIncreaseHeatImpact = 1                   // Range: 0-20
state.modelDecreaseHeatImpact = 2                   // Range: 0-20
state.modelIncreaseTemperatureImpact = 1            // Range: 0-10
state.modelDecreaseTemperatureImpact = 1            // Range: 0-10
state.threshholdModelIncreaseTemperature = 3        // Range: 1-20
state.threshholdModelDecreaseTemperature = 3        // Range: 1-20

state.maximumTemperature = 12                       // Range: 1-50
state.trueMaximumTemperature = 15                   // Range: 1-50 (must be >= maximumTemperature)
state.minimumTemperature = 1                        // Range: 1-50
state.trueMinimumTemperature = 1                    // Range: 1-50 (must be <= minimumTemperature)

state.overheatTimer = 4                             // Range: 1-50
state.overheatReductionForHeat = 5                  // Range: 0-100
state.overheatReductionForTemperature = 1           // Range: 0-10
state.cooldownTimer = 5                             // Range: 1-50
state.cooldownRate = 2                              // Range: 0-10

state.randomExplosionChance = 3                     // Range: 0-100 (%)
state.randomExplosionHeatIncreaseValue = 5          // Range: 0-50
state.randomExplosionTemperatureIncreaseValue = 2   // Range: 0-10

state.heatFloorEnabled = true                       // Boolean - Prevent excessive negative heat
state.heatFloorValue = -20                          // Range: -100-0 - Minimum heat value

// HP (Health Points) Settings
state.enableHPSystem = true                         // Boolean
state.initialHP = 100                               // Range: 1-1000
state.maximumHP = 100                               // Range: 1-1000
state.hpCriticalThreshold = 25                      // Range: 0-maximumHP
state.hpLowThreshold = 50                           // Range: 0-maximumHP
state.hpInjuredThreshold = 75                       // Range: 0-maximumHP

state.playerInjuryImpact = 10                       // Range: 0-100
state.modelInjuryImpact = 5                         // Range: 0-100
state.playerHealingImpact = 5                       // Range: 0-100
state.modelHealingImpact = 3                        // Range: 0-100
state.threshholdPlayerInjury = 1                    // Range: 0-20
state.threshholdPlayerHealing = 1                   // Range: 0-20
state.threshholdModelInjury = 2                     // Range: 0-20
state.threshholdModelHealing = 2                    // Range: 0-20

state.enableWhippoorwill = true                     // Boolean
state.whippoorwillActivationThreshold = 25          // Range: 0-100 (%)
state.whippoorwillIntensityMultiplier = 1           // Range: 0.1-10

// SP (Sanity Points) Settings
state.enableSPSystem = true                         // Boolean
state.initialSP = 100                               // Range: 1-1000
state.maximumSP = 100                               // Range: 1-1000
state.spCriticalThreshold = 25                      // Range: 0-maximumSP
state.spUnstableThreshold = 50                      // Range: 0-maximumSP
state.spShakenThreshold = 75                        // Range: 0-maximumSP

state.playerSanityLossImpact = 8                    // Range: 0-100
state.playerSanityGainImpact = 5                    // Range: 0-100
state.modelSanityLossImpact = 4                     // Range: 0-100
state.modelSanityGainImpact = 3                     // Range: 0-100
state.threshholdPlayerSanityLoss = 1                // Range: 0-20
state.threshholdPlayerSanityGain = 1                // Range: 0-20
state.threshholdModelSanityLoss = 2                 // Range: 0-20
state.threshholdModelSanityGain = 2                 // Range: 0-20

state.hallucinationChance = 15                      // Range: 0-100 (%)
state.delusionChance = 10                           // Range: 0-100 (%)

// Grace Period Settings (Horror Prevention)
state.enableGracePeriod = true                      // Boolean
state.gracePeriodDuration = 10                      // Range: 0-100
state.gracePeriodPreventsDread = true               // Boolean
state.gracePeriodPreventsInjury = true              // Boolean
state.gracePeriodPreventsSanityLoss = true          // Boolean
state.gracePeriodMaxTemperature = 3                 // Range: 1-50

// Author's Note Lock Settings
state.authorsNoteLockDuration = 5                   // Range: 0-20
...
function validateAndClampConfig() {
  ...
  // Temperature & Heat
  state.initialHeatValue = clamp(state.initialHeatValue, 0, 100)
  state.initialTemperatureValue = clamp(state.initialTemperatureValue, 1, 50)
  state.temperatureIncreaseChance = clamp(state.temperatureIncreaseChance, 1, 100)
  state.heatIncreaseValue = clamp(state.heatIncreaseValue, 0, 10)
  state.temperatureIncreaseValue = clamp(state.temperatureIncreaseValue, 0, 5)

  state.playerIncreaseHeatImpact = clamp(state.playerIncreaseHeatImpact, 0, 20)
  state.playerDecreaseHeatImpact = clamp(state.playerDecreaseHeatImpact, 0, 20)
  state.playerIncreaseTemperatureImpact = clamp(state.playerIncreaseTemperatureImpact, 0, 10)
  state.playerDecreaseTemperatureImpact = clamp(state.playerDecreaseTemperatureImpact, 0, 10)
  state.threshholdPlayerIncreaseTemperature = clamp(state.threshholdPlayerIncreaseTemperature, 1, 20)
  state.threshholdPlayerDecreaseTemperature = clamp(state.threshholdPlayerDecreaseTemperature, 1, 20)

  state.modelIncreaseHeatImpact = clamp(state.modelIncreaseHeatImpact, 0, 20)
  state.modelDecreaseHeatImpact = clamp(state.modelDecreaseHeatImpact, 0, 20)
  state.modelIncreaseTemperatureImpact = clamp(state.modelIncreaseTemperatureImpact, 0, 10)
  state.modelDecreaseTemperatureImpact = clamp(state.modelDecreaseTemperatureImpact, 0, 10)
  state.threshholdModelIncreaseTemperature = clamp(state.threshholdModelIncreaseTemperature, 1, 20)
  state.threshholdModelDecreaseTemperature = clamp(state.threshholdModelDecreaseTemperature, 1, 20)

  state.maximumTemperature = clamp(state.maximumTemperature, 1, 50)
  state.trueMaximumTemperature = clamp(state.trueMaximumTemperature, 1, 50)
  state.minimumTemperature = clamp(state.minimumTemperature, 1, 50)
  state.trueMinimumTemperature = clamp(state.trueMinimumTemperature, 1, 50)

  // Ensure logical relationships
  if (state.trueMaximumTemperature < state.maximumTemperature) {
    state.trueMaximumTemperature = state.maximumTemperature
  }
  if (state.trueMinimumTemperature > state.minimumTemperature) {
    state.trueMinimumTemperature = state.minimumTemperature
  }

  state.overheatTimer = clamp(state.overheatTimer, 1, 50)
  state.overheatReductionForHeat = clamp(state.overheatReductionForHeat, 0, 100)
  state.overheatReductionForTemperature = clamp(state.overheatReductionForTemperature, 0, 10)
  state.cooldownTimer = clamp(state.cooldownTimer, 1, 50)
  state.cooldownRate = clamp(state.cooldownRate, 0, 10)

  state.randomExplosionChance = clamp(state.randomExplosionChance, 0, 100)
  state.randomExplosionHeatIncreaseValue = clamp(state.randomExplosionHeatIncreaseValue, 0, 50)
  state.randomExplosionTemperatureIncreaseValue = clamp(state.randomExplosionTemperatureIncreaseValue, 0, 10)

  state.heatFloorEnabled = bool(state.heatFloorEnabled)
  state.heatFloorValue = clamp(state.heatFloorValue, -100, 0)

  // HP System
  state.enableHPSystem = bool(state.enableHPSystem)
  state.initialHP = clamp(state.initialHP, 1, 1000)
  state.maximumHP = clamp(state.maximumHP, 1, 1000)
  state.hpCriticalThreshold = clamp(state.hpCriticalThreshold, 0, state.maximumHP)
  state.hpLowThreshold = clamp(state.hpLowThreshold, 0, state.maximumHP)
  state.hpInjuredThreshold = clamp(state.hpInjuredThreshold, 0, state.maximumHP)

  // Ensure logical HP thresholds (two-pass validation)
  ...
  // Second pass to catch cases where first adjustment created new conflicts
  if (state.hpCriticalThreshold > state.hpLowThreshold) {
    state.hpCriticalThreshold = Math.floor(state.hpLowThreshold * 0.5)
  }

  state.playerInjuryImpact = clamp(state.playerInjuryImpact, 0, 100)
  state.modelInjuryImpact = clamp(state.modelInjuryImpact, 0, 100)
  state.playerHealingImpact = clamp(state.playerHealingImpact, 0, 100)
  state.modelHealingImpact = clamp(state.modelHealingImpact, 0, 100)
  state.threshholdPlayerInjury = clamp(state.threshholdPlayerInjury, 0, 20)
  state.threshholdPlayerHealing = clamp(state.threshholdPlayerHealing, 0, 20)
  state.threshholdModelInjury = clamp(state.threshholdModelInjury, 0, 20)
  state.threshholdModelHealing = clamp(state.threshholdModelHealing, 0, 20)

  state.enableWhippoorwill = bool(state.enableWhippoorwill)
  state.whippoorwillActivationThreshold = clamp(state.whippoorwillActivationThreshold, 0, 100)
  state.whippoorwillIntensityMultiplier = clamp(state.whippoorwillIntensityMultiplier, 0.1, 10)

  // SP System
  state.enableSPSystem = bool(state.enableSPSystem)
  state.initialSP = clamp(state.initialSP, 1, 1000)
  state.maximumSP = clamp(state.maximumSP, 1, 1000)
  state.spCriticalThreshold = clamp(state.spCriticalThreshold, 0, state.maximumSP)
  state.spUnstableThreshold = clamp(state.spUnstableThreshold, 0, state.maximumSP)
  state.spShakenThreshold = clamp(state.spShakenThreshold, 0, state.maximumSP)

  // Ensure logical SP thresholds (two-pass validation)
  if (state.spCriticalThreshold > state.spUnstableThreshold) {
    state.spCriticalThreshold = Math.floor(state.spUnstableThreshold * 0.5)
  }
  if (state.spUnstableThreshold > state.spShakenThreshold) {
    state.spUnstableThreshold = Math.floor(state.spShakenThreshold * 0.67)
  }
  // Second pass to catch cases where first adjustment created new conflicts
  if (state.spCriticalThreshold > state.spUnstableThreshold) {
    state.spCriticalThreshold = Math.floor(state.spUnstableThreshold * 0.5)
  ...
  state.playerSanityLossImpact = clamp(state.playerSanityLossImpact, 0, 100)
  state.playerSanityGainImpact = clamp(state.playerSanityGainImpact, 0, 100)
  state.modelSanityLossImpact = clamp(state.modelSanityLossImpact, 0, 100)
  state.modelSanityGainImpact = clamp(state.modelSanityGainImpact, 0, 100)
  state.threshholdPlayerSanityLoss = clamp(state.threshholdPlayerSanityLoss, 0, 20)
  state.threshholdPlayerSanityGain = clamp(state.threshholdPlayerSanityGain, 0, 20)
  state.threshholdModelSanityLoss = clamp(state.threshholdModelSanityLoss, 0, 20)
  state.threshholdModelSanityGain = clamp(state.threshholdModelSanityGain, 0, 20)

  state.hallucinationChance = clamp(state.hallucinationChance, 0, 100)
  state.delusionChance = clamp(state.delusionChance, 0, 100)

  // Grace Period
  state.enableGracePeriod = bool(state.enableGracePeriod)
  state.gracePeriodDuration = clamp(state.gracePeriodDuration, 0, 100)
  state.gracePeriodPreventsDread = bool(state.gracePeriodPreventsDread)
  state.gracePeriodPreventsInjury = bool(state.gracePeriodPreventsInjury)
  state.gracePeriodPreventsSanityLoss = bool(state.gracePeriodPreventsSanityLoss)
  state.gracePeriodMaxTemperature = clamp(state.gracePeriodMaxTemperature, 1, 50)

  // Other
  state.authorsNoteLockDuration = clamp(state.authorsNoteLockDuration, 0, 20)

  state.enableGenreDetection = bool(state.enableGenreDetection)
  state.enableLocationTracking = bool(state.enableLocationTracking)
  state.enableBonepokeSystem = bool(state.enableBonepokeSystem)
}
...
function getStoryCardEntry(cardKey) {
  // Safely retrieve a story card's entry value by its key
...
function countWords(text, wordList) {
  if (!text || !wordList || wordList.length === 0) return 0
  ...
  words.forEach(word => {
    ...
    if (fixedWord && wordList.includes(fixedWord)) {
      count++
    }
  })
...
function scoreText(text, words) {
  if (!text || !words || words.length === 0) return 0
...
function getRankedTones(text, dictionary) {
  if (!text || !dictionary) return "None"
...
const modifier = (text) => {
  ...
  /* --- INITIALIZATION --- */
  if (typeof state.heat === 'undefined' || state.heat === null) {
  ...
  // Create PlayersAuthorsNote story card on turn 1 (first execution)
  ...
  if (state.authorsNoteLockCounter > 0 && !isPlayerTurn) {
    if (state.memory && state.memory.authorsNote !== null) {
  ...
  if (state.enableLocationTracking) {
    const loreLibrary = (typeof state.loreData !== 'undefined' && state.loreData !== null) ? state.loreData : null
    ...
    if (loreLibrary && loreLibrary.locations) {
      ...
      for (const locKey in loreLibrary.locations) {
        if (loreLibrary.locations.hasOwnProperty(locKey)) {
          const location = loreLibrary.locations[locKey]
          if (location.keys && location.keys.length > 0) {
            const found = location.keys.some(key => lowerCurrentText.includes(key))
            if (found) {
  ...
  if (!state.cooldownMode) {
    ...
    if (calmingCount > 0) {
      ...
      // Apply heat floor to prevent excessive negative heat
      if (state.heatFloorEnabled && state.heat < state.heatFloorValue) {
        state.heat = state.heatFloorValue
      }
  ...
  if (state.enableSPSystem) {
    ...
    const canLoseSanity = !state.gracePeriodActive || !state.gracePeriodPreventsSanityLoss
  ...
  if (state.enableBonepokeSystem) {
    ...
    if (bonepokeResult.includes("lift")) {
      ...
      // Reset to configured base value instead of hardcoded 15
      state.temperatureIncreaseChance = state.baseTemperatureIncreaseChance
    }
  ...
  /* --- HP STATUS EFFECTS --- */
  let hpStatus = ""
  // Don't show HP status during grace period to avoid confusion
  ...
  /* --- SP STATUS EFFECTS --- */
  let spStatus = ""
  // Don't show SP status during grace period to avoid confusion
  if (state.enableSPSystem && state.sp < state.maximumSP && !state.gracePeriodActive) {
    if (state.sp <= state.spCriticalThreshold) {
